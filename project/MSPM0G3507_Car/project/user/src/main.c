...
int main (void)
{
    ...
    car_log_write("MPU-6500 init...\r\n");
    mpu6050_yaw_init();
    system_delay_ms(100);
    mpu6050_yaw_calibrate(MPU6500_CALIB_SAMPLES);
    if(mpu6050_yaw_is_ready())
    {
        car_log_write("MPU-6500 calibration done.\r\n");
    }
    else
    {
    ...
    if(mpu6050_yaw_is_ready())
    {
        ...
        car_log_write("Angle-loop car ready. IMU=MPU-6500, Yaw active.\r\n");
        car_log_write("Yaw calibration scale=1.268 (installed vehicle).\r\n");
    }
    else
    {
